...
namespace CFAPInventoryView.Services
{
    public static class SelectListBuilder
    {
        ...
        {
            var query = await context.AgeGroups.AsNoTracking().OrderBy(ag => ag.SortOrder).ToListAsync();
        ...
        {
            var query = await context.Ethnicities.AsNoTracking().OrderBy(e => e.Description).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await context.Genders.AsNoTracking().OrderBy(g => g.Description).ToListAsync();
            if ( query is null) return null;
        ...
        {
            var query = await context.Categories.AsNoTracking().OrderBy(c => c.Name).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await context.OptionalCategories.AsNoTracking().OrderBy(c => c.Name).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await context.ExcludeCategories.AsNoTracking().OrderBy(c => c.Name).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await context.Donors.AsNoTracking().OrderBy(c => c.LastName).ThenBy(c => c.FirstName).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await context.Recipients.AsNoTracking().OrderBy(c => c.LastName).ThenBy(c => c.FirstName).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await context.Baskets.AsNoTracking().Where(b => !b.IsShoppingListItem).ToListAsync();
            if (query is null) return null;
        ...
        {
            var query = await HelperMethods.GetAllAvailableSupplies(context);
            if (query is null) return null;
        ...
        {
            var query = await context.StorageLocations.AsNoTracking().OrderBy(s => s.Name).ToListAsync();
            if (query is null) return null;
