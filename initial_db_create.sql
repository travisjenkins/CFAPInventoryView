...
BEGIN
    IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'SupplyId', N'Barcode', N'CategoryId', N'Description', N'ExcludeCategoryId', N'ExpirationDate', N'Expires', N'LastUpdateDateTime', N'LastUpdateId', N'Name', N'OptionalCategoryId', N'Price', N'PurchaseDate', N'Quantity', N'StorageLocationId', N'Active', N'DonorId') AND [object_id] = OBJECT_ID(N'[Supplies]'))
        SET IDENTITY_INSERT [Supplies] ON;
    INSERT INTO [Supplies] ([SupplyId], [Barcode], [CategoryId], [Description], [ExcludeCategoryId], [ExpirationDate], [Expires], [LastUpdateDateTime], [LastUpdateId], [Name], [OptionalCategoryId], [Price], [PurchaseDate], [Quantity], [StorageLocationId], [Active], [DonorId])
